...
const generateToken = async (
  ...
  let user = await Admin.findByIdAndUpdate(id, update_data, {
    new: true,
  })
    .select("name avatar phone email uuid")
    .lean();
...
module.exports.login = async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    return response.errorResponse(res, errorArray);
  }
  ...
  try {
    ...
    if (!user) {
      ...
      return response.errorResponse(res, { msg: msg }, msg, 401);
    }
    ...
    if (!validPassword) {
      msg = "Invalid Credentials.";
      return response.errorResponse(res, { msg }, msg, 401, false);
    }

    let update_data = { last_login: Date.now() };

    if (!user.uuid) {
      update_data["uuid"] = randomUUID();
    }

    generateToken(res, user._id, update_data);
  } catch (err) {
    console.log(err);
...
module.exports.checkAuth = async (req, res) => {
  try {
    const user = await Admin.findById(req.user.id)
      .select("name avatar phone email uuid last_login createdAt")
      .lean();
